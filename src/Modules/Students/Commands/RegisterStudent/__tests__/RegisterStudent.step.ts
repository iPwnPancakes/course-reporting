...
} from '../../../Repositories/StudentRepository/InMemoryStudentRepository/InMemoryStudentRepository';
...
Then(/I should get back a Student named (.*) with email (.*)/, function (name: string, email: string) {
    if (response.ok) {
        const student: Student = response.value.get<Student>('student');
        expect(response.ok).to.equal(true);
        expect(student.getName()).to.equal(name);
        expect(student.getEmail()).to.equal(email);
    } else {
